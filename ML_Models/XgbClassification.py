...
class XgbClassification():
    ...
    def train_data_model(self):
        return self.__train_data_model

    @property
    def num_split_cross_validation(self):
        return self.__num_split_cross_validation
    ...
    def get_confusion_matrix_model(self, predictions):
        ...
        confusion_matrix_normalize = pd.crosstab(test_y, predictions, rownames=['original'], colnames=['Predicted'],
                                                 normalize=True)
        confusion_matrix_with_type = pd.DataFrame(columns=["year", "TP", "FP", "FN", "TN"],
                                                  data=[[self.__year_test, 0, 0, 0, 0]], index=[0])
    ...
    def get_accuracy_result(self, accuracy_mean, accuracy):
        ...
        result = pd.DataFrame(index=[0], columns=["year", "model_accuracy", "test_accuracy"],
                              data=[[self.__year_test, accuracy_mean, accuracy]])
        return result
